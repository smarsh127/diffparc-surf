...
est.fit(img,n_iter=snakemake.params.n_iter,model=snakemake.params.model,omp_nthreads=snakemake.threads)

os.mkdir(out_affine_dir)
for i,aff in enumerate(img.em_affines):
    aff.to_filename(os.path.join(out_affine_dir,f'gradvol-{i:03d}_desc-itk_affine.txt'),fmt='itk')
